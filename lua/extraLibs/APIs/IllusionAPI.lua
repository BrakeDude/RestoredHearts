...
local function load(prevData)
			...
			for index, item in ipairs(history) do
				if item:IsTrinket() then
					...
					if not BlackListTrinket(id) and itemTrinket then
						if
							not illusionPlayer:HasTrinket(id)
							and player:HasTrinket(id)
							and not itemTrinket:HasCustomTag("revive")
							and not itemTrinket:HasCustomTag("reviveeffect")
						then
	...
	function IllusionMod.GetTablesData()
		return {
			...
			ForbiddenTrinkets = ForbiddenTrinkets,
			ForbiddenPCombos = ForbiddenPCombos,
			EntityData = EntityData,
			CanPlaceBomb = IllusionMod.CanPlaceBomb,
			PerfectIllusion = IllusionMod.PerfectIllusion,
		}
	end
	...
	function IllusionMod.LoadCallbacks()
		for _, callback in pairs(IllusionCallbacks) do
			if callback.Priority ~= nil then
				IllusionMod:AddPriorityCallback(
					callback.Callback,
					callback.Priority,
					callback.Function,
					table.unpack(callback.Param)
				)
			else
	...
	function IllusionMod:ModLoad()
		if not IllusionMod.Loaded then
			Log("[" .. IllusionMod.Name .. "] is loaded. Version " .. IllusionMod.Version)
	...
	local function UpdateClones(_, p)
		...
		if data.IsIllusion then
			...
			if p:IsDead() then
				--p.Visible = false
				if
					p:GetPlayerType() ~= PlayerType.PLAYER_THELOST
					and p:GetPlayerType() ~= PlayerType.PLAYER_THELOST_B
					and p:GetPlayerType() ~= PlayerType.PLAYER_THESOUL_B
				then
				...
				if
					p:GetPlayerType() ~= PlayerType.PLAYER_THELOST
					and p:GetPlayerType() ~= PlayerType.PLAYER_THELOST_B
					and p:GetPlayerType() ~= PlayerType.PLAYER_THESOUL
					and p:GetPlayerType() ~= PlayerType.PLAYER_THESOUL_B
					and p:GetPlayerType() ~= PlayerType.PLAYER_THEFORGOTTEN_B
					and not p:GetEffects():HasNullEffect(NullItemID.ID_LOST_CURSE)
				then
					p:ChangePlayerType(PlayerType.PLAYER_THELOST)
	...
	local function CloneColor(_, p, _)
		...
		if d.IsIllusion then
			...
			if
				...
				and not (
					p:GetPlayerType() == PlayerType.PLAYER_THEFORGOTTEN
					or REPENTOGON and p:GetHealthType() == HealthType.BONE
				)
			then
				p:AddBoneHearts(-p:GetBoneHearts())
			end
	...
	local function AfterDeath(_, e)
		if e then
			if e:ToPlayer() and e:ToPlayer():GetPlayerType() ~= PlayerType.PLAYER_THESOUL_B or e:ToFamiliar() then
	...
	local function ClonesControls(_, entity, hook, action)
		if entity ~= nil and entity.Type == EntityType.ENTITY_PLAYER and not IllusionMod.CanPlaceBomb then
			...
			if d.IsIllusion then
				if
					(hook == InputHook.GET_ACTION_VALUE or hook == InputHook.IS_ACTION_PRESSED)
					and p:GetSprite():IsPlaying("Appear")
				then
					return hook == InputHook.GET_ACTION_VALUE and 0 or false
				end
				if
					hook == InputHook.IS_ACTION_TRIGGERED
					and (
						action == ButtonAction.ACTION_BOMB
						or action == ButtonAction.ACTION_PILLCARD
						or action == ButtonAction.ACTION_ITEM
						or p:GetSprite():IsPlaying("Appear")
					)
				then
					return false
				end
...
if IllusionMod then
	if IllusionMod.Version < localversion or not IllusionMod.Loaded then
		if not IllusionMod.Loaded then
			Log("[" .. IllusionMod.Name .. "] Reloading...")
		else
			Log(
				"["
					.. IllusionMod.Name
