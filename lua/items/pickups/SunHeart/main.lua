...
function ComplianceSunLocal:SunHeartCollision(pickup, collider)
	if
		...
		and pickup.SubType == RestoredHearts.Enums.Pickups.Hearts.HEART_SUN
	then
...
function ComplianceSunLocal:PreEternalSpawn(pickup)
	if
		RestoredHearts:GetDefaultFileSave("SunHeartSpawnChance") > 0
		and TSIL.Random.GetRandom(pickup.InitSeed)
			>= (1 - RestoredHearts:GetDefaultFileSave("SunHeartSpawnChance") / 100)
	then
...
RestoredHearts:AddCallback(
	...
	ComplianceSunLocal.PreEternalSpawn,
	{ PickupVariant.PICKUP_HEART, HeartSubType.HEART_ETERNAL }
)

---@param pickup EntityPickup
function ComplianceSunLocal:PreSunSpawn(pickup)
	if
		RestoredHearts:GetDefaultFileSave("SunHeartSpawnChance") <= 0
	then
		pickup:Morph(
...
RestoredHearts:AddPriorityCallback(
	TSIL.Enums.CustomCallback.POST_PICKUP_INIT_FIRST,
	1,
	ComplianceSunLocal.PreSunSpawn,
	{ PickupVariant.PICKUP_HEART, RestoredHearts.Enums.Pickups.Hearts.HEART_SUN }
)
...
function ComplianceSunLocal:SunClear(rng, pos)
	for i = 0, Game():GetNumPlayers() - 1 do
		...
		if ComplianceSun.GetSunHeartsNum(player) > 0 then
			for slot = 0, 2 do
				if
					player:GetActiveItem(slot) ~= nil
					and player:GetActiveItem(slot) ~= CollectibleType.COLLECTIBLE_ALABASTER_BOX
				then
					...
					if itemConfig and itemConfig.ChargeType ~= 2 then
						...
						if charge < battery and newcharge > 0 then
							...
							sfx:Play(RestoredHearts.Enums.SFX.Hearts.SUN_PICKUP, 1, 0)
							local BatteryEffect = Isaac.Spawn(
