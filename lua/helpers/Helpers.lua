...
local function TryRemoveStoreCredit(player)
	if Game():GetRoom():GetType() == RoomType.ROOM_SHOP then
		if player:HasTrinket(TrinketType.TRINKET_STORE_CREDIT) then
			...
			TSIL.Players.GetPlayersByTrinket(TrinketType.TRINKET_STORE_CREDIT)
			for _, player in ipairs(TSIL.Players.GetPlayersByTrinket(TrinketType.TRINKET_STORE_CREDIT)) do
				RemoveStoreCreditFromPlayer(player)
				return
			end
		end
...
function Helpers.AddCharge(player, amount, slot, force)
	...
	if REPENTOGON then
		...
		player:SetActiveCharge(
			math.min(charges + amount, itemConfig.MaxCharges * Helpers.BatteryChargeMult(player)),
			slot
		)
